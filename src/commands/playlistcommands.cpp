...
ClearCommand::ClearCommand(PlaylistModel& model, QUndoCommand* parent) : QUndoCommand(parent), m_model(model) {
...
NewBinCommand::NewBinCommand(PlaylistModel& model, QTreeWidget* tree, const QString& bin, QUndoCommand* parent)
...
RenameBinCommand::RenameBinCommand(PlaylistModel& model, QTreeWidget* tree, const QString& bin, const QString& newName,
                                   QUndoCommand* parent)
    : QUndoCommand(parent), m_model(model), m_binTree(tree), m_bin(bin), m_newName(newName) {
	if (newName.isEmpty()) {
