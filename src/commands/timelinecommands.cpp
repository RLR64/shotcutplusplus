...
LiftCommand::LiftCommand(MultitrackModel& model, int trackIndex, int clipIndex, QUndoCommand* parent)
    : QUndoCommand(parent), m_model(model), m_trackIndex(qBound(0, trackIndex, qMax(model.rowCount() - 1, 0))),
      m_clipIndex(clipIndex), m_undoHelper(m_model) {
...
RemoveCommand::RemoveCommand(MultitrackModel& model, MarkersModel& markersModel, int trackIndex, int clipIndex,
                             QUndoCommand* parent)
    : QUndoCommand(parent), m_model(model), m_markersModel(markersModel),
      m_trackIndex(qBound(0, trackIndex, qMax(model.rowCount() - 1, 0))), m_clipIndex(clipIndex), m_undoHelper(m_model),
      m_rippleAllTracks(Settings.timelineRippleAllTracks()), m_rippleMarkers(Settings.timelineRippleMarkers()),
      m_markerRemoveStart(-1), m_markerRemoveEnd(-1) {
...
GroupCommand::GroupCommand(MultitrackModel& model, QUndoCommand* parent) : QUndoCommand(parent), m_model(model) {
}
...
UngroupCommand::UngroupCommand(MultitrackModel& model, QUndoCommand* parent) : QUndoCommand(parent), m_model(model) {
}
...
CompositeTrackCommand::CompositeTrackCommand(MultitrackModel& model, int trackIndex, bool value, QUndoCommand* parent)
    : QUndoCommand(parent), m_model(model), m_trackIndex(qBound(0, trackIndex, qMax(model.rowCount() - 1, 0))),
      m_value(value), m_oldValue(model.data(m_model.index(trackIndex), MultitrackModel::IsCompositeRole).toBool()) {
...
LockTrackCommand::LockTrackCommand(MultitrackModel& model, int trackIndex, bool value, QUndoCommand* parent)
    : QUndoCommand(parent), m_model(model), m_trackIndex(qBound(0, trackIndex, qMax(model.rowCount() - 1, 0))),
      m_value(value), m_oldValue(model.data(m_model.index(trackIndex), MultitrackModel::IsLockedRole).toBool()) {
...
MoveClipCommand::MoveClipCommand(TimelineDock& timeline, int trackDelta, int positionDelta, bool ripple,
    ...
    : QUndoCommand(parent), m_timeline(timeline), m_model(*timeline.model()), m_markersModel(*timeline.markersModel()),
      ...
      m_rippleAllTracks(Settings.timelineRippleAllTracks()), m_rippleMarkers(Settings.timelineRippleMarkers()),
      m_undoHelper(m_model), m_redo(false), m_earliestStart(-1) {
	m_undoHelper.SetHints(UndoHelper::RestoreTracks);
	m_undoHelper.RecordBeforeState();
}
...
void MoveClipCommand::AddClip(int trackIndex, int clipIndex) {
	...
	if (info && info->cut) {
		...
		saveInfo.m_uuid = MLT.ensureHasUuid(*info->producer);
...
void MoveClipCommand::Redo() {
		...
		if (track) {
			...
			if (targetIndex >= clipIndex || // pushing clips on same track
				...
				if (done) {
					...
					RedoMarkers();
					selection = m_timeline.uuidsToSelection(QVector<QUuid>() << m_clips.first().m_uuid);
	...
	for (auto& clip : m_clips) {
		...
		auto info = m_model.findClipByUuid(clip.m_uuid, trackIndex, clipIndex);
		if (info && info->producer && info->producer->is_valid() && info->cut) {
			if (m_ripple)
...
bool MoveClipCommand::MergeWith(const QUndoCommand* other) {
	...
	while (thisIterator != m_clips.end() && thatIterator != that->m_clips.end()) {
		if (thisIterator.value().m_uuid != thatIterator.value().m_uuid)
			return false;
		thisIterator++;
		thatIterator++;
	}
...
TrimClipInCommand::TrimClipInCommand(MultitrackModel& model, MarkersModel& markersModel, int trackIndex, int clipIndex,
                                     int delta, bool ripple, bool redo, QUndoCommand* parent)
    : TrimCommand(parent), m_model(model), m_markersModel(markersModel),
      m_trackIndex(qBound(0, trackIndex, qMax(model.rowCount() - 1, 0))), m_clipIndex(clipIndex), m_delta(delta),
      m_ripple(ripple), m_rippleAllTracks(Settings.timelineRippleAllTracks()),
      m_rippleMarkers(Settings.timelineRippleMarkers() && m_ripple), m_redo(redo) {
...
TrimClipOutCommand::TrimClipOutCommand(MultitrackModel& model, MarkersModel& markersModel, int trackIndex,
                                       int clipIndex, int delta, bool ripple, bool redo, QUndoCommand* parent)
    : TrimCommand(parent), m_model(model), m_markersModel(markersModel),
      m_trackIndex(qBound(0, trackIndex, qMax(model.rowCount() - 1, 0))), m_clipIndex(clipIndex), m_delta(delta),
      m_ripple(ripple), m_rippleAllTracks(Settings.timelineRippleAllTracks()),
      m_rippleMarkers(Settings.timelineRippleMarkers() && m_ripple), m_redo(redo) {
...
void SplitCommand::Redo() {
	LOG_DEBUG() << "trackIndex" << m_trackIndex[0] << "clipIndex" << m_clipIndex[0] << "position" << m_position;
	MAIN.filterController()->pauseUndoTracking();
	m_undoHelper.RecordBeforeState();
...
AddTransitionCommand::AddTransitionCommand(TimelineDock& timeline, int trackIndex, int clipIndex, int position,
    ...
    : QUndoCommand(parent), m_timeline(timeline), m_model(*m_timeline.model()),
      ...
      m_rippleAllTracks(Settings.timelineRippleAllTracks()), m_rippleMarkers(Settings.timelineRippleMarkers()),
      m_markerOldStart(-1), m_markerNewStart(-1) {
...
TrimTransitionOutCommand::TrimTransitionOutCommand(MultitrackModel& model, int trackIndex, int clipIndex, int delta,
                                                   bool redo, QUndoCommand* parent)
    : TrimCommand(parent), m_model(model), m_trackIndex(qBound(0, trackIndex, qMax(model.rowCount() - 1, 0))),
      m_clipIndex(clipIndex), m_delta(delta), m_notify(false), m_redo(redo) {
...
AddTrackCommand::AddTrackCommand(MultitrackModel& model, bool isVideo, QUndoCommand* parent)
    : QUndoCommand(parent), m_model(model), m_isVideo(isVideo) {
	if (isVideo)
...
InsertTrackCommand::InsertTrackCommand(MultitrackModel& model, int trackIndex, TrackType trackType,
	...
	if (trackType != AudioTrackType && trackType != VideoTrackType) {
		m_trackType = model.trackList().size() > 0 ? model.trackList().at(m_trackIndex).type : VideoTrackType;
	}
...
RemoveTrackCommand::RemoveTrackCommand(MultitrackModel& model, int trackIndex, QUndoCommand* parent)
    : QUndoCommand(parent), m_model(model), m_trackIndex(qBound(0, trackIndex, qMax(model.rowCount() - 1, 0))),
      m_trackType(model.trackList().at(m_trackIndex).type), m_undoHelper(model) {
	if (m_trackType == AudioTrackType)
...
MoveTrackCommand::MoveTrackCommand(MultitrackModel& model, int fromTrackIndex, int toTrackIndex, QUndoCommand* parent)
    : QUndoCommand(parent), m_model(model), m_fromTrackIndex(qBound(0, fromTrackIndex, qMax(model.rowCount() - 1, 0))),
...
UpdateCommand::UpdateCommand(TimelineDock& timeline, int trackIndex, int clipIndex, int position, QUndoCommand* parent)
    : QUndoCommand(parent), m_timeline(timeline), m_trackIndex(trackIndex), m_clipIndex(clipIndex),
      m_position(position), m_isFirstRedo(true), m_undoHelper(*timeline.model()), m_ripple(Settings.timelineRipple()),
...
void DetachAudioCommand::Redo() {
	...
	if (audioClip.is_valid() && videoClip.is_valid()) {
		...
		// Save the group number if it exists
		{
			...
			if (videoClipInfo && videoClipInfo->cut && videoClipInfo->cut->property_exists(kShotcutGroupProperty)) {
...
AlignClipsCommand::AlignClipsCommand(MultitrackModel& model, QUndoCommand* parent)
    : QUndoCommand(parent), m_model(model), m_undoHelper(m_model), m_redo(false) {
	m_undoHelper.SetHints(UndoHelper::RestoreTracks);
	m_undoHelper.RecordBeforeState();
...
void AlignClipsCommand::AddAlignment(QUuid uuid, int offset, double speed) {
	Alignment alignment;
	alignment.m_uuid   = uuid;
	alignment.m_offset = offset;
	alignment.m_speed  = speed;
...
void ApplyFiltersCommand::Undo() {
	...
	for (auto& clip : m_prevFilters.keys()) {
		...
		if (clipInfo && clipInfo->producer) {
			...
			Mlt::Producer previousProducer(MLT.profile(), "xml-string", m_prevFilters[clip].toUtf8().constData());
