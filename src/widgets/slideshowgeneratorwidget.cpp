...
SlideshowGeneratorWidget::SlideshowGeneratorWidget(Mlt::Playlist* clips, QWidget* parent)
	...
	m_aspectConversionCombo->setCurrentIndex(Settings.slideshowAspectConversion(ASPECT_CONVERSION_CROP_CENTER));
...
auto SlideshowGeneratorWidget::getSlideshow() -> Mlt::Playlist* {
	...
	for (int i = 0; i < count; i++) {
		...
		if (c && c->producer && c->producer->is_valid()) {
			auto isAudioVideo = QString::fromLatin1(c->producer->get("mlt_service")).startsWith("avformat");
			auto maxFrames =
			    qRound(MLT.profile().fps() * (isAudioVideo ? config.audioVideoDuration : config.imageDuration));
...
void SlideshowGeneratorWidget::applyLumaTransitionProperties(Mlt::Transition* luma, SlideshowConfig& config) {
	...
	if (index == cutIndex) {
		...
		luma->set("resource", QStringLiteral("%luma%1.pgm").arg(index - 2, 2, 10, QChar('0')).toLatin1().constData());
