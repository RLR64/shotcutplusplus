...
OpenGLVideoWidget::OpenGLVideoWidget(QObject* parent)
    : VideoWidget{parent}, m_quickContext(nullptr), m_isThreadedOpenGL(false) {
...
void OpenGLVideoWidget::initialize() {
	...
	auto context = static_cast<QOpenGLContext*>(
	    quickWindow()->rendererInterface()->getResource(quickWindow(), QSGRendererInterface::OpenGLContextResource));
	...
	const auto nvidia = QString::fromUtf8((const char*)glGetString(GL_RENDERER)).toLower().contains("nv");
...
static void uploadTextures(QOpenGLContext* context, const SharedFrame& frame, GLuint texture[]) {
	...
	f->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	check_error(f);
	f->glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, width / 2, height / 2, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE,
	                image + width * height);
	check_error(f);
...
void OpenGLVideoWidget::renderVideo() {
	...
	if (rect().width() > 0.0 && zoom() > 0.0) {
		if (offset().x() || offset().y())
			modelView.translate(-offset().x() * devicePixelRatioF(), offset().y() * devicePixelRatioF());
