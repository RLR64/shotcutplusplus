...
GlaxnimateProducerWidget::GlaxnimateProducerWidget(QWidget* parent)
    : QWidget(parent), ui(new Ui::GlaxnimateProducerWidget) {
	ui->setupUi(this);
...
static void modifyJsonValue(QJsonValue& destValue, const QString& path, const QJsonValue& newValue) {
	...
	const int arrayIndex =
	    path.mid(indexOfSquareBracketOpen + 1, indexOfSquareBracketClose - indexOfSquareBracketOpen - 1).toInt();

	const QString squareBracketPropertyName = path.left(indexOfSquareBracketOpen);
	const QString squareBracketSubPath =
	    indexOfSquareBracketClose > 0
	        ? (path.mid(indexOfSquareBracketClose + 1)[0] == '.' ? path.mid(indexOfSquareBracketClose + 2)
...
void GlaxnimateIpcServer::ParentResources::setProducer(const Mlt::Producer& producer, bool hideCurrentTrack) {
	m_producer = producer;
...
void GlaxnimateIpcServer::onReadyRead() {
	...
	if (m_isProtocolValid && parent) {
		...
		if (frameNum != parent->m_frameNum) {
			...
			if (!parent || !parent->m_glaxnimateProducer || !parent->m_glaxnimateProducer->is_valid() || time < 0.0) {
				MLT.seek(frameNum);
				return;
			}
...
void GlaxnimateIpcServer::launch(const Mlt::Producer& producer, QString filename, bool hideCurrentTrack) {
	...
	if (Settings.playerGPU()) {
		...
		connect(qobject_cast<Mlt::VideoWidget*>(MLT.videoWidget()), &Mlt::VideoWidget::frameDisplayed, this,
	...
	if (!childProcess.startDetached()) {
		...
		QMessageBox dialog(QMessageBox::Information, qApp->applicationName(),
		                   ...
		                   QMessageBox::Ok | QMessageBox::Cancel, MAIN.window());
