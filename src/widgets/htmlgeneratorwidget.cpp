...
#include "jobs/htmlgeneratorjob.hpp"
...
HtmlGeneratorWidget::HtmlGeneratorWidget(QWidget* parent) : QWidget(parent), ui(new Ui::HtmlGeneratorWidget) {
	ui->setupUi(this);
	...
	QFile::copy(QStringLiteral(":/resources/html/icons/defaults.webp"), dir.filePath(tr("(defaults)") + ".webp"));
	...
	QFile::copy(QStringLiteral(":/resources/html/elastic_stroke/icon.webp"), dir.filePath(name + ".webp"));
	...
	p.set(kColorProperty, colorStringToResource(kTransparent).toLatin1().constData());
	ui->preset->savePreset(p, name);
	QFile::copy(QStringLiteral(":/resources/html/gold_metal/icon.webp"), dir.filePath(name + ".webp"));
	...
	p.set(kColorProperty, colorStringToResource(kTransparent).toLatin1().constData());
	ui->preset->savePreset(p, name);
	QFile::copy(QStringLiteral(":/resources/html/party_time/icon.webp"), dir.filePath(name + ".webp"));
	...
	p.set(kColorProperty, colorStringToResource(kTransparent).toLatin1().constData());
	ui->preset->savePreset(p, name);
	QFile::copy(QStringLiteral(":/resources/html/neon_flux/icon.webp"), dir.filePath(name + ".webp"));
...
auto HtmlGeneratorWidget::getPreset() const -> Mlt::Properties {
	...
	p.set(kJavaScriptProperty, ui->javascriptTextEdit->toPlainText().toUtf8().toBase64().constData());
...
void HtmlGeneratorWidget::setProducer(Mlt::Producer* p) {
	...
	if (!p || !p->is_valid())
		return;
	const auto   mltColor = p->property_exists(kColorProperty) ? p->get_color(kColorProperty) : mlt_color({0, 0, 0, 0});
	const QColor qtColor(mltColor.r, mltColor.g, mltColor.b, mltColor.a);
	ui->colorLabel->setText(colorToString(qtColor));
	ui->colorLabel->setStyleSheet(
	    QStringLiteral("color: %1; background-color: %2").arg(Util::textColor(qtColor), qtColor.name()));
	const auto css = QByteArray::fromBase64(p->get(kCssProperty));
	ui->cssTextEdit->setPlainText(QString::fromUtf8(css));
	const auto body = QByteArray::fromBase64(p->get(kBodyProperty));
	ui->bodyTextEdit->setPlainText(QString::fromUtf8(body));
...
%4
  ...
  <script>%2</script>
	...
	if (formattedBody.contains("%3")) {
		formattedBody =
		    formattedBody.arg(ui->line1LineEdit->text(), ui->line2LineEdit->text(), ui->line3LineEdit->text());
	...
	return html.arg(ui->cssTextEdit->toPlainText(), ui->javascriptTextEdit->toPlainText(), ui->colorLabel->text(),
	                formattedBody);
}
...
void HtmlGeneratorWidget::on_imageButton_clicked() {
	...
	if (outputPath.isEmpty()) {
		outputPath =
		    QFileDialog::getSaveFileName(this, tr("Generate Image File"), Settings.savePath(), tr("PNG files (*.png)"));
		if (outputPath.isEmpty())
			return;
...
void HtmlGeneratorWidget::on_videoButton_clicked() {
	...
	if (outputPath.isEmpty()) {
		outputPath =
		    QFileDialog::getSaveFileName(this, tr("Generate Video File"), Settings.savePath(), tr("AVI files (*.avi)"));
		if (outputPath.isEmpty())
			return;
	...
	job->setProperty(kJavaScriptProperty, ui->javascriptTextEdit->toPlainText().toUtf8().toBase64().constData());
...
void HtmlGeneratorWidget::on_presetIconView_itemClicked(QListWidgetItem* item) {
	...
	std::unique_ptr<Mlt::Properties> properties{Mlt::Properties::parse_yaml(presetPath.toUtf8().constData())};
	if (!properties)
		return;
	loadPreset(*properties);
