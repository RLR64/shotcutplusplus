...
void AvformatProducerWidget::offerConvert(QString message, bool set709Convert, bool setSubClip) {
	...
	dialog.setSubClipChecked(setSubClip &&
...
void AvformatProducerWidget::keyPressEvent(QKeyEvent* event) {
	if (ui->speedSpinBox->hasFocus() && (event->key() == Qt::Key_Enter || event->key() == Qt::Key_Return)) {
...
void AvformatProducerWidget::recreateProducer(bool getFrame) {
	...
	if (!p || !p->is_valid()) {
		LOG_ERROR() << "failed to recreate producer for:" + Util::GetFilenameFromProducer(producer());
		return;
	}
	...
	for (int i = 0; i < n; i++) {
		...
		if (streamType == "video") {
			...
			if (QString::number(audioIndex) == m_producer->get(kAudioIndexProperty)) {
				...
				ui->audioTableWidget->setItem(3, 1, new QTableWidgetItem(m_producer->get(key.toLatin1().constData())));
	...
	if (fps() != 0.0) {
		ui->videoTableWidget->setItem(
		    ...
		    new QTableWidgetItem(
		        QStringLiteral("%L1 %2").arg(fps(), 0, 'f', 6).arg(isVariableFrameRate ? tr("(variable)") : "")));
	}
...
void AvformatProducerWidget::on_audioTrackComboBox_activated(int index) {
	if (m_producer) {
		...
		m_producer->set(kAudioIndexProperty, ui->audioTrackComboBox->itemData(index).toString().toUtf8().constData());
...
void AvformatProducerWidget::on_reverseButton_clicked() {
	...
	if (result == QDialog::Accepted) {
		...
		if (!filename.isEmpty()) {
			...
			MeltJob* meltJob = new MeltJob(filename, meltArgs, m_producer->get_int("meta.media.frame_rate_num"),
...
void AvformatProducerWidget::on_actionExtractSubclip_triggered() {
	...
	if (!filename.isEmpty()) {
		...
		if (JOBS.targetIsInProgress(filename)) {
			QMessageBox::warning(this, caption, QObject::tr("A job already exists for %1").arg(filename));
			return;
		}
		...
		if (m_producer->get_int(kFilterInProperty) || m_producer->get_int("in")) {
			if (m_producer->get(kFilterInProperty))
				ffmpegArgs
				    << "-ss"
				    << QString::fromLatin1(m_producer->get_time(kFilterInProperty, mlt_time_clock)).replace(',', '.');
			else
				ffmpegArgs << "-ss"
		...
		if (m_producer->get(kFilterOutProperty))
			ffmpegArgs
			    << "-to"
			    << QString::fromLatin1(m_producer->get_time(kFilterOutProperty, mlt_time_clock)).replace(',', '.');
		else
			ffmpegArgs << "-to" << QString::fromLatin1(m_producer->get_time("out", mlt_time_clock)).replace(',', '.');
...
void AvformatProducerWidget::on_actionBitrateViewer_triggered() {
	...
	if (m_producer->get_int("video_index") >= 0)
		...
		args << "-select_streams" << QString::fromLatin1("a:%1").arg(m_producer->get_int(kAudioIndexProperty));
	args << "-show_entries"
