...
void ImageProducerWidget::updateDuration() {
	if (m_producer->get(kFilterOutProperty))
		ui->durationSpinBox->setValue(m_producer->get_int(kFilterOutProperty) - m_producer->get_int(kFilterInProperty) +
		                              1);
	else
...
void ImageProducerWidget::recreateProducer() {
	...
	if (m_watcher) {
		...
		connect(m_watcher.get(), &QFileSystemWatcher::fileChanged, this, &ImageProducerWidget::recreateProducer);
	}
}
	...
	if (checked) {
		...
		if (count) {
			...
			m_producer->set("length",
			                m_producer->frames_to_time(imageCount * m_producer->get_int("ttl"), mlt_time_clock));
