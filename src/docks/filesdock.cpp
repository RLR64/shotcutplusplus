...
static const QSet<QString> kImageExtensions{
    QLatin1String("jpg"),  QLatin1String("jpeg"), QLatin1String("png"),  QLatin1String("bmp"), QLatin1String("tif"),
    QLatin1String("tiff"), QLatin1String("svg"),  QLatin1String("webp"), QLatin1String("gif"), QLatin1String("tga"),
};
...
static const QSet<QString> kVideoExtensions{
    QLatin1String("mp4"),  QLatin1String("m4v"), QLatin1String("avi"),  QLatin1String("mpg"),
    QLatin1String("mpeg"), QLatin1String("ts"),  QLatin1String("mts"),  QLatin1String("m2ts"),
    QLatin1String("mkv"),  QLatin1String("ogv"), QLatin1String("webm"), QLatin1String("dv"),
    QLatin1String("lrv"),  QLatin1String("360"), QLatin1String("flv"),  QLatin1String("wmv"),
};

static void cacheMediaType(FilesModel* model, const QString& filePath, int mediaType, const QModelIndex& index);
static void cacheThumbnail(FilesModel* model, const QString& filePath, QImage& image, const QModelIndex& index);
		...
		if (producer.is_valid()) {
			if (MLT.isImageProducer(&producer)) {
				...
				if (service.startsWith(QLatin1String("avformat"))) {
					if (producer.get_int("video_index") > -1 && Util::getSuggestedFrameRate(&producer) != 90000)
  ...
  public:
	...
	QVariant data(const QModelIndex& index, int role = Qt::DisplayRole) const override {
		...
		case ThumbnailRole: {
			...
			if (image.isNull()) {
				...
				if (!path.endsWith(QStringLiteral(".mlt"), Qt::CaseInsensitive) && !info.isShortcut())
	...
	void updateThumbnails(const QModelIndex& index) {
		...
		if (!path.endsWith(QStringLiteral(".mlt"), Qt::CaseInsensitive))
			QThreadPool::globalInstance()->start(new FilesThumbnailTask(const_cast<FilesModel*>(this), path, index));
	}
  ...
  private:
	...
	int mediaType(const QModelIndex& index) const {
		...
		if (mediaType < 0) {
			...
			m_dock->setCacheMediaType(path, mediaType);
			QThreadPool::globalInstance()->start(new FilesMediaTypeTask(const_cast<FilesModel*>(this), path, index));
		}
...
class FilesTileDelegate : public QStyledItemDelegate {
	Q_OBJECT
  public:
	FilesTileDelegate(QAbstractItemView* view, QWidget* parent = nullptr) : QStyledItemDelegate(parent), m_view(view) {
	...
	void paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
		...
		painter->drawText(
		    textPoint,
		    tr("Date: %1").arg(index.data(FilesModel::DateRole).toDateTime().toString("yyyy-MM-dd HH:mm:ss")));
		textPoint.setY(textPoint.y() + lineHeight);
		if (fileInfo.isFile()) {
...
class FilesProxyModel : public QSortFilterProxyModel {
  public:
	explicit FilesProxyModel(QObject* parent = nullptr) : QSortFilterProxyModel(parent) {
	}
...
FilesDock::FilesDock(QWidget* parent) : QDockWidget(parent), ui(new Ui::FilesDock) {
...
#if defined(Q_OS_MAC)
	ui->locationsCombo->addItem(tr("Movies", "The system-provided videos folder called Movies on macOS"),
	...
	ui->locationsCombo->addItem(
	    tr("Volumes", "The macOS file system location where external drives and network shares are mounted"),
	    "/Volumes");
#else
	...
	connect(m_filesModel, &QFileSystemModel::directoryLoaded, this, &FilesDock::updateStatus, Qt::QueuedConnection);
	connect(m_filesProxyModel, &QAbstractItemModel::modelAboutToBeReset, this, &FilesDock::clearStatus);
	...
	toolbar2->addActions({Actions["filesFiltersVideo"], Actions["filesFiltersAudio"], Actions["filesFiltersImage"],
...
void FilesDock::updateViewMode() {
	...
	m_view->setRootIndex(m_filesProxyModel->mapFromSource(m_filesModel->index(m_filesModel->rootPath())));
	m_iconsView->updateSizes();
	m_view->show();
}
...
void FilesDock::on_addLocationButton_clicked() {
	const auto path =
	    m_filesProxyModel->mapToSource(m_view->rootIndex()).data(QFileSystemModel::FilePathRole).toString();
	if (path.isEmpty())
		return;
...
void FilesDock::on_removeLocationButton_clicked() {
	...
	if (location.isEmpty())
		return;
	QMessageBox dialog(QMessageBox::Question, tr("Delete Location"),
	                   tr("Are you sure you want to remove %1?").arg(location), QMessageBox::No | QMessageBox::Yes,
	                   this);
