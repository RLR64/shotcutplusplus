...
SubtitlesDock::SubtitlesDock(QWidget* parent)
    : QDockWidget(parent), m_model(nullptr), m_pos(-1), m_textEditInProgress(false) {
	LOG_DEBUG() << "begin";
	...
	button->setIcon(QIcon::fromTheme("show-menu", QIcon(":/icons/oxygen/32x32/actions/show-menu.png")));
...
void SubtitlesDock::setupActions() {
	...
	action->setIcon(QIcon::fromTheme("subtitle", QIcon(":/icons/oxygen/32x32/actions/subtitle.png")));
	...
	action->setIcon(
	    QIcon::fromTheme("keyframes-filter-in", QIcon(":/icons/oxygen/32x32/actions/keyframes-filter-in.png")));
	...
	action->setIcon(
	    QIcon::fromTheme("keyframes-filter-out", QIcon(":/icons/oxygen/32x32/actions/keyframes-filter-out.png")));
	...
	action->setIcon(QIcon::fromTheme("4-direction", QIcon(":/icons/oxygen/32x32/actions/4-direction.png")));
	...
	action->setToolTip(tr("Create a new video track on the timeline with text showing these subtitles."));
	...
	action->setIcon(QIcon::fromTheme("speech-to-text", QIcon(":/icons/oxygen/32x32/actions/speech-to-text.png")));
	...
	action->setIcon(QIcon::fromTheme("text-speak", QIcon(":/icons/oxygen/32x32/actions/text-speak.png")));
	...
	connect(action, &QAction::toggled, this, [&](bool checked) { Settings.setSubtitlesTrackTimeline(checked); });
...
void SubtitlesDock::importSrtFromFile(const QString& srtPath, const QString& trackName, const QString& lang,
...
void SubtitlesDock::speechToText() {
	...
	whisperJob->setPostJobAction(
	    new ImportSrtPostJobAction(tmpSrt->fileName(), dialog.name(), langCode, dialog.includeNonspoken(), this));
