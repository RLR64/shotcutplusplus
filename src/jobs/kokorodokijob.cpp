...
KokorodokiJob::KokorodokiJob(const QString& inputFile, const QString& outputFile, const QString& language,
                             const QString& voice, double speed, QThread::Priority priority)
...
auto KokorodokiJob::checkDockerImage(QWidget* parent) -> bool {
	...
	if (!dockerKokorodokiExists.first) {
		QMessageBox dialog(QMessageBox::Warning, QApplication::applicationName(),
		                   ...
		                   QMessageBox::Cancel | QMessageBox::Ok, parent);
		...
		QMessageBox dialog(QMessageBox::Question, QApplication::applicationName(),
		                   parent
