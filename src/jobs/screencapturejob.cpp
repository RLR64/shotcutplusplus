...
void ScreenCaptureJob::stop() {
	...
	if (m_dbusService == DBusService::GNOME) {
		...
		QDBusMessage msg = QDBusMessage::createMethodCall("org.gnome.Shell.Screencast", "/org/gnome/Shell/Screencast",
	...
	if (m_dbusService == DBusService::GNOME) {
		...
		if (!m_actualFilename.endsWith(".mp4")) {
			...
			if (m_isAutoOpen && QFileInfo::exists(inputFileName)) {
				...
				remuxJob->setPostJobAction(new OpenPostJobAction(inputFileName, m_filename, inputFileName));
				JOBS.add(remuxJob);
...
bool ScreenCaptureJob::startGnomeScreencast() {
	...
	QDBusMessage checkMsg  = QDBusMessage::createMethodCall("org.gnome.Shell.Screencast", "/org/gnome/Shell/Screencast",
	...
	auto msg = QDBusMessage::createMethodCall("org.gnome.Shell.Screencast", "/org/gnome/Shell/Screencast",
	...
	connect(watcher, &QDBusPendingCallWatcher::finished, this, [this](QDBusPendingCallWatcher* w) {
		...
		if (reply.isError()) {
			...
			LOG_DEBUG() << "GNOME Screencast started, success:" << success << "filename:" << filename;
	...
	LOG_INFO() << "Started GNOME Shell Screencast to:" << fileInfo.path() + "/" + fileInfo.completeBaseName() + ".mkv";
...
bool ScreenCaptureJob::startKdeSpectacle() {
	...
	bool connected = bus.connect("org.kde.Spectacle", "/", "org.kde.Spectacle", "RecordingTaken", this,
	...
	connected = bus.connect("org.kde.Spectacle", "/", "org.kde.Spectacle", "RecordingFailed", this,
	                        SLOT(onDBusRecordingFailed()));
	if (!connected) {
		LOG_WARNING() << "Failed to connect to RecordingFailed signal";
		bus.disconnect("org.kde.Spectacle", "/", "org.kde.Spectacle", "RecordingTaken", this,
		               SLOT(onDBusRecordingTaken(QString)));
		return false;
	}
	...
	LOG_DEBUG() << "Recording region:" << m_rect;
	auto msg = QDBusMessage::createMethodCall("org.kde.Spectacle", "/", "org.kde.Spectacle",
...
void ScreenCaptureJob::onDBusRecordingTaken(const QString& fileName) {
	...
	bus.disconnect("org.kde.Spectacle", "/", "org.kde.Spectacle", "RecordingTaken", this,
	               SLOT(onDBusRecordingTaken(QString)));
	bus.disconnect("org.kde.Spectacle", "/", "org.kde.Spectacle", "RecordingFailed", this,
	               SLOT(onDBusRecordingFailed()));
