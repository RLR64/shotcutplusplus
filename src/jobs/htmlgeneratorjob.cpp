...
HtmlGeneratorJob::HtmlGeneratorJob(const QString& name, const QString& html, const QString& outputPath, int duration,
                                   QThread::Priority priority)
    : AbstractJob(name, priority), m_html(html), m_outputPath(outputPath), m_duration(duration), m_generator(nullptr),
      m_isGeneratingFrames(true), m_previousPercent(0) {
...
void HtmlGeneratorJob::start() {
	...
	connect(m_generator, &HtmlGenerator::imageReady, this, &HtmlGeneratorJob::onAnimationFramesReady);
	connect(m_generator, &HtmlGenerator::progressUpdate, this, &HtmlGeneratorJob::onHtmlGeneratorProgress);
...
void HtmlGeneratorJob::onAnimationFramesReady() {
	...
	QStringList args;
	args << "-r" << QString::number(fps()) << "-i" << m_tempDir->path() + "/frame_%04d.png" << "-codec:v"
	     << "utvideo"
	     << "-pix_fmt"
	     << "gbrap"
	     << "-y" << m_outputPath;
...
void HtmlGeneratorJob::onOpenTriggered() {
	...
	p->set(HtmlGeneratorWidget::kColorProperty, property(HtmlGeneratorWidget::kColorProperty).toByteArray());
	p->set(HtmlGeneratorWidget::kCssProperty, property(HtmlGeneratorWidget::kCssProperty).toByteArray());
	p->set(HtmlGeneratorWidget::kBodyProperty, property(HtmlGeneratorWidget::kBodyProperty).toByteArray());
	p->set(HtmlGeneratorWidget::kJavaScriptProperty, property(HtmlGeneratorWidget::kJavaScriptProperty).toByteArray());
	p->set(HtmlGeneratorWidget::kLine1Property, property(HtmlGeneratorWidget::kLine1Property).toByteArray());
	p->set(HtmlGeneratorWidget::kLine2Property, property(HtmlGeneratorWidget::kLine2Property).toByteArray());
	p->set(HtmlGeneratorWidget::kLine3Property, property(HtmlGeneratorWidget::kLine3Property).toByteArray());
	MAIN.open(p, false);
}
