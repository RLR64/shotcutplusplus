...
void ScreenCapture::startRectangleRecording() {
	...
	connect(m_rectangleSelector.get(), &RectangleSelector::rectangleSelected, this,
	        &ScreenCapture::onRectangleSelected);
	connect(m_rectangleSelector.get(), &RectangleSelector::canceled, this, &ScreenCapture::onSelectionCanceled);
...
void ScreenCapture::startWindowRecording() {
	...
	connect(m_windowPicker.get(), &WindowPicker::windowSelected, this, &ScreenCapture::onWindowSelected);
	m_windowPicker->show();
}
...
void ScreenCapture::startWindowSnapshot() {
	...
	m_windowPicker = std::make_unique<WindowPicker>();
	connect(m_windowPicker.get(), &WindowPicker::windowSelected, this, &ScreenCapture::onImageWindowSelected);
	m_windowPicker->show();
#endif
}
...
bool ScreenCapture::captureImagePortal(const QRect& rect, const QString& outputPath) {
	...
	bool ok = bus.connect("org.freedesktop.portal.Desktop", handlePath.path(), "org.freedesktop.portal.Request",
	...
	responseLoop.exec();
	m_portalEventLoop = nullptr;
	bus.disconnect("org.freedesktop.portal.Desktop", handlePath.path(), "org.freedesktop.portal.Request",
	               QStringLiteral("Response"), this, SLOT(onPortalResponse(uint, QVariantMap)));
...
QRect ScreenCapture::adjustRectForVideo(const QRect& rect) {
	...
	if (result != rect)
		LOG_DEBUG() << "Adjusted capture area from" << rect << "to" << result << "(H.264 requires even dimensions)";
	return result;
}
