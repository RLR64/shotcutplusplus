...
ScreenCaptureToolbar::ScreenCaptureToolbar(bool isRecordingMode, QWidget* parent)
	...
	// Style buttons
	QString const buttonStyle = "QPushButton {"
	...
	// Style checkboxes
	QString const checkboxStyle = "QCheckBox {"
	                        "  color: white;"
	                        "  font-size: 12px;"
	                        "  spacing: 5px;"
	                        "}"
	...
	// Connect signals
	connect(m_fullscreenButton, &QPushButton::clicked, this, &ScreenCaptureToolbar::onFullscreenClicked);
	connect(m_rectangleButton, &QPushButton::clicked, this, &ScreenCaptureToolbar::onRectangleClicked);
	...
	if (screen) {
		QRect const screenGeometry = screen->geometry();
		adjustSize();
		const int x = screenGeometry.center().x() - width() / 2;
		const int y = screenGeometry.top() + 20;
		move(x, y);
	}
}

ScreenCaptureToolbar::~ScreenCaptureToolbar() = default;
...
void ScreenCaptureToolbar::onFullscreenClicked() {
	emit captureModeSelected(static_cast<int>(ScreenCapture::Fullscreen), m_minimizeCheckbox->isChecked(),
	                         m_audioCheckbox->isChecked());
	close();
}

void ScreenCaptureToolbar::onRectangleClicked() {
	emit captureModeSelected(static_cast<int>(ScreenCapture::Rectangle), m_minimizeCheckbox->isChecked(),
	                         m_audioCheckbox->isChecked());
	close();
}

void ScreenCaptureToolbar::onWindowClicked() {
	emit captureModeSelected(static_cast<int>(ScreenCapture::Window), m_minimizeCheckbox->isChecked(),
	                         m_audioCheckbox->isChecked());
	close();
}
