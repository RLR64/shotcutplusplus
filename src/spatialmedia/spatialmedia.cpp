...
static constexpr uint8_t SPHERICAL_UUID_ID[] = {0xff, 0xcc, 0x82, 0x63, 0xf8, 0x55, 0x4a, 0x93,
...
static const std::string SPHERICAL_XML_HEADER = "<?xml version=\"1.0\"?>"
                                          "<rdf:SphericalVideo\n"
                                          "xmlns:rdf=\"http://www.w3.org/1999/02/22-rdf-syntax-ns#\"\n"
                                          "xmlns:GSpherical=\"http://ns.google.com/videos/1.0/spherical/\">";

static const std::string SPHERICAL_XML_CONTENTS =
...
static const std::string SPHERICAL_XML_CONTENTS_CROP_FORMAT =
...
static const std::string SPHERICAL_XML_FOOTER = "</rdf:SphericalVideo>";

// static std::string SPHERICAL_PREFIX = "{http://ns.google.com/videos/1.0/spherical/}";

static auto spherical_uuid(std::string& strMetadata) -> Box* {
	...
	const int iSize = strMetadata.size();
	const auto* pMetadata = reinterpret_cast<const uint8_t*>(strMetadata.c_str());
...
static auto get_descriptor_length(std::fstream& inFile) -> int {
	auto    result = 0;
	uint8_t size_byte;
...
static auto get_aac_num_channels(Box* mp4aBox, std::fstream& inFile) -> int {
	auto result = -1;
	auto size   = sizeof(mp4aBox->m_name);
	auto pos    = inFile.tellg();

	for (auto box : dynamic_cast<Container*>(mp4aBox)->m_listContents) {
...
static auto sound_samples_contains(const char* name) -> bool {
...
static auto get_sample_description_num_channels(Box* ssdBox, std::fstream& inFile) -> int {
	auto result = -1;
...
static void mpeg4_add_spatial_audio(Box* mdiaBox, std::fstream& inFile) {
	...
	for (const auto box : dynamic_cast<Container*>(mdiaBox)->m_listContents) {
		if (!memcmp(constants::TAG_MINF, box->m_name, size)) {
			for (const auto box : dynamic_cast<Container*>(box)->m_listContents) {
					...
					for (const auto box : dynamic_cast<Container*>(box)->m_listContents) {
						if (!memcmp(constants::TAG_STSD, box->m_name, size)) {
							for (const auto box : dynamic_cast<Container*>(box)->m_listContents) {
								...
								if (4 == channels) {
									dynamic_cast<Container*>(box)->add(SA3DBox::create(channels));
									break;
								}
							}
						}
...
static auto mpeg4_add_spherical(Mpeg4Container* pMPEG4, std::fstream& inFile, std::string& strMetadata) -> bool {
	...
	bool bAdded = false;
	auto* pMoov = dynamic_cast<Container*>(pMPEG4->m_pMoovBox);
	if (!pMoov)
		return false;

	auto it = pMoov->m_listContents.begin();
	while (it != pMoov->m_listContents.end()) {
		Container* pBox = dynamic_cast<Container*>(*it++);
		if (memcmp(pBox->m_name, constants::TAG_TRAK, 4) == 0) {
			bAdded = false;
			pBox->remove(constants::TAG_UUID);

			auto it2 = pBox->m_listContents.begin();
			while (it2 != pBox->m_listContents.end()) {
				Container* pSub = dynamic_cast<Container*>(*it2++);
				if (memcmp(pSub->m_name, constants::TAG_MDIA, 4) != 0)
					continue;

				auto it3 = pSub->m_listContents.begin();
				while (it3 != pSub->m_listContents.end()) {
					...
					char name[4];
					const int iPos = pMDIA->content_start() + 8;
					inFile.seekg(iPos);
					inFile.read(name, 4);
...
auto SpatialMedia::injectSpherical(const std::string& strInFile, const std::string& strOutFile) -> bool {
	...
	;
	const bool bRet = mpeg4_add_spherical(pMPEG4, inFile, xml);
	if (!bRet) {
