...
static constexpr int kMaxImageDurationSecs{3600 * 4};
...
class Controller {
  ...
  public:
	...
	virtual void close();
	[[nodiscard]] virtual int displayWidth() const  = 0;
	[[nodiscard]] virtual int displayHeight() const = 0;
	...
	bool saveXML(const QString& filename, Service* service = nullptr, bool withRelativePaths = true, QTemporaryFile* tempFile = nullptr, bool proxy = false, const QString& projectNote = QString());
	...
	static void copyFilters(Mlt::Producer& fromProducer, Mlt::Producer& toProducer, bool fromClipboard = false, int filterIndex = FILTER_INDEX_ENABLED);
	...
	static void adjustFilter(Mlt::Filter* filter, int in, int out, int inDelta, int outDelta, int keyframeDelta);
	static void adjustClipFilters(Mlt::Producer& producer, int in, int out, int inDelta, int outDelta, int keyframeDelta);

	[[nodiscard]] bool hasFiltersOnClipboard() const {
	...
	[[nodiscard]] ShotcutSettings::ProcessingMode processingMode() const {
		return m_processingMode;
	}

	[[nodiscard]] Mlt::Repository* repository() const {
		return m_repo;
	}
	...
	[[nodiscard]] Mlt::Consumer* consumer() const {
	...
	[[nodiscard]] const TransportControllable* transportControl() const {
		return &m_transportControl;
	}

	[[nodiscard]] Mlt::Producer* savedProducer() const {
