...
// Number constants
static constexpr double setSecondtoFloatNumber{15000.0};

static auto getPrefix(const QString& name, const QString& value) -> QString;

static auto isMltClass(const QString& name) -> bool {
	return name == "profile" || name == "producer" || name == "filter" || name == "playlist" || name == "tractor" ||
	       name == "track" || name == "transition" || name == "consumer" || name == "chain" || name == "link";
}

static auto isNetworkResource(const QString& string) -> bool {
	// Check if it looks like a qualified URL. Try parsing it and see.
	QRegularExpression const schemaTest(QLatin1String("^[a-zA-Z]{2,}\\:.*"));
	// Not actually checking network URL due to latency and transience.
	return (schemaTest.match(string).hasMatch() && QUrl(string).isValid() && !string.startsWith("plain:"));
}

static auto isNumericProperty(const QString& name) -> bool {
	return name == "length" || name == "geometry" || name == "rect" || name == "warp_speed";
}

MltXmlChecker::MltXmlChecker()
    : m_needsGPU(false), m_needsCPU(false), m_hasEffects(false), m_isConverted(false), m_isCorrected(false),
      m_isUpdated(false), m_decimalPoint('.'), m_numericValueChanged(false), m_isTractorTransition(false) {
	m_unlinkedFilesModel.setColumnCount(ColumnCount);
}

auto MltXmlChecker::check(const QString& fileName) -> QXmlStreamReader::Error {
	LOG_DEBUG() << "begin";
	...
	if (file.open(QIODevice::ReadOnly | QIODevice::Text) && m_tempFile->open()) {
		...
		if (m_xml.readNextStartElement()) {
			if (m_xml.name().toString() == "mlt") {
				...
				foreach (QXmlStreamAttribute const a, m_xml.attributes()) {
					if (a.name().toString().toUpper() == "LC_NUMERIC") {
						QString const value = a.value().toString().toUpper();
						...
						m_newXml.writeAttribute(a.name().toString(), "Shotcut version 26.2 ");
...
auto MltXmlChecker::errorString() const -> QString {
...
void MltXmlChecker::processProperties() {
	...
	if (mlt_class == "filter" || mlt_class == "transition" || mlt_class == "producer" || mlt_class == "chain" ||
	    mlt_class == "link") {
		...
		const bool proxyEnabled = Settings.proxyEnabled();
		if (proxyEnabled)
			checkForProxy(mlt_service, newProperties);

		// Second pass: amend property values.
		const bool relinkMismatch =
		    !m_resource.hash.isEmpty() && !m_resource.newHash.isEmpty() && m_resource.hash != m_resource.newHash;
	...
	foreach (MltProperty const p, newProperties) {
		m_newXml.writeStartElement("property");
...
auto MltXmlChecker::checkNumericString(QString& value) -> bool {
...
auto MltXmlChecker::fixWebVfxPath(QString& resource) -> bool {
	...
	if (fi.isAbsolute() || Util::hasDriveLetter(resource)) {
		QDir const appPath(QCoreApplication::applicationDirPath());
		...
		if (!resource.startsWith(appPath.path())) {
			...
			const int i = resource.indexOf("/share/shotcut/");
#if defined(Q_OS_MAC)
			if (i == -1)
...
static auto getSuffix(const QString& name, const QString& value) -> QString {
...
auto MltXmlChecker::readResourceProperty(const QString& name, QString& value) -> bool {
	if (mlt_class == "filter" || mlt_class == "transition" || mlt_class == "producer" || mlt_class == "chain" ||
	    mlt_class == "link")
		if (name == "resource" || name == "src" || name == "filename" || name == "luma" || name == "luma.resource" ||
		    name == "composite.luma" || name == "producer.resource" || name == "av.file" || name == "warp_resource") {
...
void MltXmlChecker::checkUnlinkedFile(const QString& mlt_service) {
	...
	if (!m_unlinkedFilesModel.findItems(filePath, Qt::MatchFixedString | Qt::MatchCaseSensitive).isEmpty())
		return;

	LOG_ERROR() << "file not found: " << m_resource.info.filePath();
	QIcon const icon(":/icons/oxygen/32x32/status/task-reject.png");
	auto* item = new QStandardItem(icon, filePath);
	item->setToolTip(item->text());
	item->setData(m_resource.hash, ShotcutHashRole);
	m_unlinkedFilesModel.appendRow(item);
}

auto MltXmlChecker::fixUnlinkedFile(QString& value) -> bool {
...
auto MltXmlChecker::fixVersion1701WindowsPathBug(QString& value) -> bool {
...
void MltXmlChecker::replaceMovitServices(QString& mlt_service, QVector<MltProperty>& properties) {
	...
	if (Settings.playerGPU()) {
		...
		if (mlt_service == "movit.blur") {
			...
			for (auto& p : properties) {
				if (p.first == "fill" || p.first == "distort" || p.first == "rect" || p.first == "valign" ||
				    p.first == "halign")
			...
			for (auto& p : properties) {
				if (p.first == "neutral_color") {
					...
					p.first  = "1";
					p.second = QString::number(p.second.toFloat() / setSecondtoFloatNumber);
				}
			}
		} else {
			properties.removeIf([](const auto& prop) -> auto { return prop.first.startsWith("movit."); });
		}
	}
			...
			for (auto& p : properties) {
				if (p.first == "resource") {
					...
					if (QFileInfo(pathName).isRelative()) {
						QDir const projectDir(QFileInfo(m_tempFile->fileName()).dir());
...
void MltXmlChecker::checkForProxy(const QString& mlt_service, QVector<MltXmlChecker::MltProperty>& properties) {
	const bool isTimewarp = mlt_service == "timewarp";
	if (mlt_service.startsWith("avformat") || isTimewarp) {
		...
		QDir const proxyDir(Settings.proxyFolder());
		QDir projectDir(QFileInfo(m_tempFile->fileName()).dir());
		QString const fileName = hash + ProxyManager::videoFilenameExtension();
		...
		QDir const proxyDir(Settings.proxyFolder());
		QDir projectDir(QFileInfo(m_tempFile->fileName()).dir());
		QString const fileName = hash + ProxyManager::imageFilenameExtension();
		projectDir.cd("proxies");
		if (proxyDir.exists(fileName) || projectDir.exists(fileName)) {
			::utime(proxyDir.filePath(fileName).toUtf8().constData(), nullptr);
			::utime(projectDir.filePath(fileName).toUtf8().constData(), nullptr);
...
auto MltXmlChecker::checkMltVersion() -> bool {
