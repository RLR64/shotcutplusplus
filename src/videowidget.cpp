...
VideoWidget::VideoWidget(QObject* parent)
	...
	connect(quickWindow(), &QQuickWindow::visibilityChanged, this, &VideoWidget::setBlankScene, Qt::QueuedConnection);
...
auto VideoWidget::reconfigure(bool isMulti) -> int {
	...
	if (m_consumer->is_valid()) {
		...
		const bool isDeckLinkHLG  = serviceName.startsWith("decklink") && property("decklinkGamma").toInt() == 1;
		...
		case ShotcutSettings::Linear10GpuCpu:
			...
			break;
		default: // Native8Cpu
			m_consumer->set("mlt_image_format", serviceName.startsWith("decklink") ? "yuv422" : "yuv420p");
			break;
		}
		...
		if (processingMode == ShotcutSettings::Linear10Cpu ||
		    (processingMode == ShotcutSettings::Linear10GpuCpu && property("decklinkGamma").toInt() != 1)) {
		...
		if (isMulti) {
			...
			m_consumer->set("0.deinterlacer", property("deinterlacer").toString().toLatin1().constData());
			...
			m_consumer->set("deinterlacer", property("deinterlacer").toString().toLatin1().constData());
		...
		if (m_glslManager) {
			if (!m_threadCreateEvent)
				m_threadCreateEvent.reset(
				    m_consumer->listen("consumer-thread-create", this, (mlt_listener)onThreadCreate));
...
RenderThread::RenderThread(thread_function_t function, void* data)
    : QThread{nullptr}, m_function{function}, m_data{data}, m_context{new QOpenGLContext},
...
FrameRenderer::FrameRenderer() : QThread(nullptr), m_semaphore(3), m_imageRequested(false) {
