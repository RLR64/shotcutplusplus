...
TranscribeAudioDialog::TranscribeAudioDialog(const QString& trackName, QWidget* parent) : QDialog(parent) {
	...
	m_table->setColumnWidth(ExtensionModel::COLUMN_SIZE,
	                        fontMetrics().horizontalAdvance("XXX.XX XXX") * devicePixelRatioF() + 12);
	connect(m_table, &QAbstractItemView::clicked, this, &TranscribeAudioDialog::onModelRowClicked);
	m_table->setContextMenuPolicy(Qt::CustomContextMenu);
	connect(m_table, &QWidget::customContextMenuRequested, this, &TranscribeAudioDialog::showModelContextMenu);
...
void TranscribeAudioDialog::onModelRowClicked(const QModelIndex& index) {
	if (!m_model.downloaded(index.row())) {
		QMessageBox qDialog(QMessageBox::Question, tr("Download Model"),
...
void TranscribeAudioDialog::showEvent(QShowEvent* event) {
	...
	QMessageBox qDialog(QMessageBox::Question, tr("Download Model"), tr("No models found. Download a standard model?"),
	                    QMessageBox::No | QMessageBox::Yes, this);
	qDialog.setDefaultButton(QMessageBox::Yes);
	qDialog.setEscapeButton(QMessageBox::No);
	qDialog.setWindowModality(QmlApplication::dialogModality());
...
void TranscribeAudioDialog::refreshModels(bool report) {
	...
	if (dlDialog.start() && report) {
		m_model.load(QmlExtension::WHISPER_ID);
		QMessageBox qDialog(QMessageBox::Information, tr("Refresh Models"), tr("Models refreshed"), QMessageBox::Ok,
		                    this);
		qDialog.setWindowModality(QmlApplication::dialogModality());
		qDialog.exec();
	} else if (report) {
		QMessageBox qDialog(QMessageBox::Critical, tr("Refresh Models"), tr("Failed to refresh models"),
		                    QMessageBox::Ok, this);
		qDialog.setWindowModality(QmlApplication::dialogModality());
		qDialog.exec();
	}
