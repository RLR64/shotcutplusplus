...
void MetadataModel::updateFilterMask(bool isClipProducer, bool isChainProducer, bool isTrackProducer,
...
void MetadataModel::saveFilterSet(const QString& name) {
	...
	if (file.error() != QFileDevice::NoError) {
		LOG_ERROR() << "error while writing filter set file" << file.fileName() << ":" << file.errorString();
		return;
	}
