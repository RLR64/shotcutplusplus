...
PlaylistModel::PlaylistModel(QObject* parent)
    : QAbstractTableModel(parent), m_playlist(nullptr), m_dropRow(-1), m_mode(Invalid) {
	qRegisterMetaType<QVector<int>>("QVector<int>");
	connect(this, &QAbstractItemModel::rowsAboutToBeRemoved, this, &PlaylistModel::onRowsAboutToBeRemoved);
}
...
auto PlaylistModel::data(const QModelIndex& index, int role) const -> QVariant {
	...
	if (info) {
		...
		case FIELD_DURATION:
			if (info->producer && info->producer->is_valid()) {
				return QString::fromLatin1(info->producer->frames_to_time(info->frame_count, Settings.timeFormat()));
			} else {
				return "";
			}
		case FIELD_START:
...
void PlaylistModel::setBin(int row, const QString& name) {
	...
	if (producer && producer->is_valid()) {
		...
		emit dataChanged(createIndex(row, PlaylistModel::COLUMN_BIN), createIndex(row, PlaylistModel::COLUMN_BIN));
		emit modified();
	}
}
...
void PlaylistModel::renameBin(const QString& bin, const QString& newName) {
	if (newName.isEmpty()) {
		emit dataChanged(createIndex(0, PlaylistModel::COLUMN_BIN), createIndex(rowCount(), PlaylistModel::COLUMN_BIN));
		emit modified();
		return;
	}
	...
	if (m_playlist && m_playlist->is_valid()) {
		for (int i = 0; i < m_playlist->count(); i++) {
			...
			if (info && info->producer && info->producer->is_valid()) {
				QThreadPool::globalInstance()->start(new UpdateThumbnailTask(this, *info->producer, info->frame_in,
				                                                             info->frame_out, i),
				                                     1);
			}
			delete info;
...
void PlaylistModel::setInOut(int row, int in, int out) {
	...
	if (info && info->producer && info->producer->is_valid()) {
		...
		QThreadPool::globalInstance()->start(new UpdateThumbnailTask(this, *info->producer, in, out, row), 1);
