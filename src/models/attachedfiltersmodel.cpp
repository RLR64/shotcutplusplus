...
bool AttachedFiltersModel::setData(const QModelIndex& index, const QVariant&, int role) {
	...
	if (filter && filter->is_valid()) {
		...
		if (isSourceClip()) {
			...
			MAIN.undoStack()->push(new Filter::DisableCommand(*this, name(index.row()), index.row(), !disabled));
		}
	} else {
...
int AttachedFiltersModel::add(QmlMetadata* meta) {
	...
	case QmlMetadata::Filter: {
		...
		if (filter.is_valid()) {
			...
			filter.set_in_and_out(m_producer->get(kFilterInProperty) ? m_producer->get_int(kFilterInProperty)
			                                                         : m_producer->get_in(),
	...
	case QmlMetadata::Link: {
		...
		if (meta->seekReverse() && m_producer->get_int("meta.media.has_b_frames") != 0) {
			emit requestConvert(tr("This file has B-frames, which is not supported by %1.").arg(meta->name()), false,
			                    true);
			return -1;
		}
		...
		if (link.is_valid()) {
			...
			link.set_in_and_out(m_producer->get(kFilterInProperty) ? m_producer->get_int(kFilterInProperty)
			                                                       : m_producer->get_in(),
			                    m_producer->get(kFilterOutProperty) ? m_producer->get_int(kFilterOutProperty)
			                                                        : m_producer->get_out());
			if (isSourceClip()) {
...
void AttachedFiltersModel::reset(Mlt::Producer* producer) {
	...
	if (m_producer && m_producer->is_valid()) {
		Mlt::Event* event =
		    m_producer->listen("service-changed", this, (mlt_listener)AttachedFiltersModel::producerChanged);
		m_event.reset(event);
		int count = 0;
		if (m_producer->type() == mlt_service_chain_type) {
