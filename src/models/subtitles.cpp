...
int Subtitles::indexForTime(const Subtitles::SubtitleVector& items, int64_t msTime, int searchStart, int msMargin) {
	...
	} else if (searchStart > -1 && searchStart < count && (items[searchStart].start - msMargin) <= msTime &&
	...
	} else if (searchStart > -1 && (searchStart + 1) < count && (items[searchStart + 1].start - msMargin) <= msTime &&
