...
// Number constants
static constexpr int setScreenSnapshotNumber{42};
static constexpr int setScreenRecordingNumber{43};

OpenOtherDialog::OpenOtherDialog(QWidget* parent) : QDialog(parent), ui(new Ui::OpenOtherDialog) {
	ui->setupUi(this);
	m_current = ui->networkWidget;
	m_addTimelineButton = ui->buttonBox->addButton(tr("Add To Timeline"), QDialogButtonBox::ApplyRole);
	connect(m_addTimelineButton, &QPushButton::clicked, this, [this]() -> void { done(-1); });
	...
	auto* group = new QTreeWidgetItem(ui->treeWidget, QStringList(tr("Network")));
	...
	if (mltProducers->get_data("decklink")) {
		auto* item = new QTreeWidgetItem(group, QStringList(tr("SDI/HDMI")));
	...
	auto* item = new QTreeWidgetItem(group, QStringList(tr("Audio/Video Device")));
...
auto OpenOtherDialog::newProducer(Mlt::Profile& profile, QObject* widget) const -> Mlt::Producer* {
	return dynamic_cast<AbstractProducerWidget*>(widget)->newProducer(profile);
}

auto OpenOtherDialog::newProducer(Mlt::Profile& profile) const -> Mlt::Producer* {
...
void OpenOtherDialog::load(Mlt::Producer* producer) {
	if (producer && producer->is_valid()) {
		...
		QString const resource(MLT.resource());
...
void OpenOtherDialog::selectTreeWidget(const QString& s) {
	for (int j = 0; j < ui->treeWidget->topLevelItemCount(); j++) {
		QTreeWidgetItem const* group = ui->treeWidget->topLevelItem(j);
...
void OpenOtherDialog::on_treeWidget_currentItemChanged(QTreeWidgetItem* current, QTreeWidgetItem*) {
	if (current->data(0, Qt::UserRole).isValid()) {
		QString const currentData(current->data(0, Qt::UserRole).toString());

#if defined(Q_OS_MAC) || defined(Q_OS_WIN)
		// Handle screen capture options
		if (currentData == "screenSnapshot") {
			done(setScreenSnapshotNumber); // Custom result code for screen snapshot
			return;
		} else if (currentData == "screenRecording") {
			done(setScreenRecordingNumber); // Custom result code for screen recording
			return;
		}
#endif
		...
		for (int i = 0; i < ui->methodTabWidget->count(); i++) {
			QString const tabName(ui->methodTabWidget->widget(i)->objectName());
			if (currentData == tabName) {
				ui->methodTabWidget->setCurrentIndex(i);
				QWidget const* w = ui->methodTabWidget->currentWidget();
				if (w == ui->networkTab) {
					m_current = ui->networkWidget;
