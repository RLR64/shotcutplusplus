 ...
 * a different log record format using the setFormat() function.
 *
 ...
 * Optional field width argument may be specified right after the command (through the colon symbol before the closing
 * bracket) Some commands requires an additional formatting argument (in the second {} brackets).
 *
 ...
 *   \arg \c %{time} - timestamp. You may specify your custom timestamp format using the second {} brackets after the
 * marker, timestamp format here will be similiar to those used in QDateTime::toString() function. For example,
 ...
 *   \arg \c %{Function} - Name of function that called on of the LOG_* macros. Uses the \c Q_FUNC_INFO macro provided
 * with Qt.
