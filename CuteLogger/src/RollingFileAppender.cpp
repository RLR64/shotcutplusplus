...
RollingFileAppender::RollingFileAppender(const QString& fileName)
    : FileAppender(fileName), m_frequency(DailyRollover), m_logFilesLimit(0) {
}
...
void RollingFileAppender::computeRollOverTime() {
	...
	Q_ASSERT_X(now.toString(m_datePatternString) == m_rollOverSuffix, "DailyRollingFileAppender::computeRollOverTime()",
